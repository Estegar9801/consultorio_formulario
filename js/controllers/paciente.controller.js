...
const crearNuevaLinea = ({ nombre, apellido, id, correo, edad, telefono, especialidad }) => {
    ...
    <table class="info-table" id=${id}>
        ...
        <tr class="info-row">
            <th class="info-column info-column-title">Edad</th>
            <th class="info-column info-column-content">${edad} años</th>
        </tr>
        <tr class="info-row">
    ...
    editarBoton.addEventListener("click", () => {
        window.location.href = `../pages/editar_paciente.html?id=${id}`;
    });
...
const crearLineaVacio = () => {
    const linea = document.createElement("div");
    const contenido = "No se encuentra ningún paciente registrado."    
    linea.innerHTML = contenido;
    return linea;
};

personaServices.listaPersonas("Paciente")
    .then((data) => {
        if(data.length !== 0) {
            data.forEach((datos) => {
